"use client";
// src/app/invoice/[orderId]/InvoiceViewer.jsx
// Q-05: Client component that auto-polls the public invoice API so the
// customer sees payment status update live (no manual refresh).
...
export default function InvoiceViewer({ orderId }) {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(`/api/invoice/${orderId}`, { cache: "no-store" });
      if (!res.ok) {
        setData(null);
        setLoading(false);
        return;
      ...
      setData(prev => {
        const newBalance = Math.max(0, (json.order?.price || 0) - (json.order?.deposit || 0) - (json.order?.paid || 0));
        if (prev && prevBalance.current !== null && prevBalance.current !== newBalance) {
          setJustUpdated(true);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          fontSize: 12, fontWeight: 600, color: isPaid ? C.green : C.sub,
          background: isPaid ? `${C.green}14` : C.s2,
          border: `1px solid ${isPaid ? `${C.green}33` : C.border2}`,
          borderRadius: 20, padding: "5px 12px",
          transition: "color 0.3s ease, background 0.3s ease",
        }}>
          <span style={{
            width: 7, height: 7, borderRadius: "50%",
            background: isPaid ? C.green : C.gold,
            animation: isPaid ? "none" : "livePulse 1.6s ease-in-out infinite",
            flexShrink: 0,
          }}/>
          {isPaid ? "Fully Paid" : justUpdated ? "Payment updated ✓" : "Updates automatically"}
        </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            transform: justUpdated ? "scale(1.12)" : "scale(1)",
